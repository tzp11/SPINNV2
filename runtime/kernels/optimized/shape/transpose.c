...
static inline void transpose4x4_neon(const float *src, int src_stride,
                                      float *dst, int dst_stride)
{
    ...
    vst1q_f32(dst, o0);
    vst1q_f32(dst + dst_stride, o1);
    vst1q_f32(dst + 2 * dst_stride, o2);
    vst1q_f32(dst + 3 * dst_stride, o3);
}
...
static void transpose_matrix(const float *src, float *dst, int rows, int cols)
{
    ...
    for (; r + 4 <= rows; r += 4) {
        int c = 0;
        for (; c + 4 <= cols; c += 4) {
            transpose4x4_neon(src + (size_t)r * cols + c, cols,
                              dst + (size_t)c * rows + r, rows);
        }
        /* Border columns: scalar 4 rows x (cols - c) */
        for (; c < cols; c++) {
...
int kernel_transpose_simd(Spkv2Context *ctx, const Spkv2NodeRecord *node, void *scratch)
{
    ...
    if (rank >= 2) {
        ...
        if (last_two_swap) {
            ...
            int row_tiles_full = rows / 4;
            int row_tail       = rows - row_tiles_full * 4;
            long total_tiles = (long)batch * row_tiles_full;
            int do_par = (long)rows * cols * (long)batch >= 32768;
            ...
            for (long t = 0; t < total_tiles; t++) {
                size_t b = (size_t)(t / row_tiles_full);
                int    r = (int)(t % row_tiles_full) * 4;
                const float *src_p = x + b * plane;
                float       *dst_p = y + b * plane;
                int c = 0;
                for (; c + 4 <= cols; c += 4) {
                    transpose4x4_neon(src_p + (size_t)r * cols + c, cols,
                                      dst_p + (size_t)c * rows + r, rows);
                }
                for (; c < cols; c++) {
                    for (int rr = 0; rr < 4; rr++)
                        dst_p[(size_t)c * rows + r + rr] =
                            src_p[(size_t)(r + rr) * cols + c];
                }
            }
            /* Tail rows (rows % 4) -- sequential, small. */
            if (row_tail > 0) {
                for (size_t b = 0; b < batch; b++) {
                    const float *src_p = x + b * plane;
                    float       *dst_p = y + b * plane;
                    int r0 = row_tiles_full * 4;
                    for (int r = r0; r < rows; r++) {
                        for (int c = 0; c < cols; c++)
                            dst_p[(size_t)c * rows + r] =
                                src_p[(size_t)r * cols + c];
                    }
                }
            }
            return 0;
